...
The results of this project suggest that the gap between output-only evaluation and trace-level evaluation is not merely theoretical — it is measurable, and in some cases the two approaches lead to opposite conclusions about an agent's production readiness. The sample RAG agent evaluated in this study scored perfectly on four of five IR metrics, produced responses rated as helpful and safe by the G-Eval judge, and completed all queries within its step budget without looping. Under any output-only evaluation framework, this would constitute a strong result. Yet the same agent received a plan success score of 0.20 and a faithfulness score of 0.00 — two failures that are entirely invisible without access to the execution trace. This divergence is the central empirical contribution of the project: it demonstrates that correct retrieval and acceptable final responses are not sufficient conditions for sound agent behavior, and that a production system could pass conventional evaluation while exhibiting structural deficiencies in how it reasons and acts.

The faithfulness-hallucination divergence deserves particular attention because it challenges a common assumption in RAG evaluation. It is generally assumed that a low hallucination score implies a grounded response. The results here show that this is not necessarily true: the agent produced factually accurate answers drawn from its parametric knowledge rather than its retrieved context, scoring 0.00 on faithfulness while scoring 0.00 on hallucination simultaneously. For general-purpose assistants this may be acceptable, but for enterprise deployments where auditability, regulatory compliance, and source traceability are requirements, an ungrounded answer is a failure regardless of its factual accuracy. AgentScope's dual-metric approach makes this distinction visible in a way that a single quality score cannot.

The adversarial results raise a different set of concerns. A policy violation rate of 0.33 — with particularly weak resistance in the instruction_override and sensitive_action categories — indicates that the agent has not been hardened against the most common social engineering attack patterns. The practical value of AgentScope here is not just that it detects this gap, but that it localizes it: rather than reporting a generic low adversarial robustness score, the per-category breakdown identifies exactly which attack patterns succeed and which do not, giving engineers a concrete starting point for remediation. The fact that unsafe_tool_use was fully resisted (3/3) suggests that tool-level guardrails are in place, but that prompt-level guardrails are not, which points toward a specific type of intervention.

There are meaningful limitations to acknowledge. The evaluation was conducted on a single representative query rather than the full 100-query ground truth set, which limits the statistical confidence of the reported scores. The ground truth CSV was derived from the agent's own test suite rather than independent human annotation, which may inflate IR scores relative to real user query distributions. The plan_success score of 0.20 likely reflects, at least in part, the G-Eval judge's limited context: it receives only the sequence of tool names without the intermediate observations that motivated each call, and may penalize sequences that are rational in execution but appear redundant when viewed as an abstract list. These limitations do not invalidate the findings, but they suggest that the scores should be interpreted as directional indicators rather than precise measurements, and that a rigorous production evaluation would require a larger, independently annotated query set evaluated over multiple runs.

Looking forward, the most significant gap in the current framework is multi-agent evaluation beyond handoff correctness. As orchestrator-specialist architectures become more common — where a coordinator agent delegates tasks to specialized sub-agents that share memory and pass state — the failure modes that matter most are conflict resolution and shared memory consistency. These are not addressable by trace-level analysis of individual tool calls; they require cross-agent state comparison and a semantic model of what correct coordination looks like. Addressing this gap is the most important direction for future work.

More broadly, this project reinforces the argument that agentic AI systems require a fundamentally different evaluation philosophy than static LLMs. A static model produces a response; an agent executes a plan. Evaluating the plan requires observing it, which requires instrumentation, and evaluating whether it was a good plan requires judgment that extends beyond surface-level response quality. AgentScope provides a practical foundation for this kind of evaluation — one that is framework-agnostic, reproducible, and deployable without disrupting the agent's own codebase. The central ambition of the project is to contribute to a broader shift in evaluation practice toward asking not only whether an agent answered correctly, but whether it did so through a sound process and in a way that can be trusted in production.

### 7.1 Future Work

Several concrete directions follow directly from the limitations and findings described above. Running the full 100-query evaluation set would provide statistically meaningful mean scores and enable query-type-specific failure analysis. Expanding the adversarial suite to incorporate recent jailbreaking benchmarks and domain-specific attack patterns would improve coverage beyond the four categories currently tested. Implementing multi-agent conflict resolution and shared memory consistency metrics would extend the framework to orchestrator-specialist architectures. Streaming evaluation — incremental trace collection for agents that emit partial results — would address long-running agents that cannot be evaluated synchronously. Completing the Modal.com cloud deployment would enable evaluation runs without local infrastructure. Finally, curating a set of reference baseline scores for common agent architectures would enable out-of-the-box calibration drift detection on first evaluation, rather than requiring a prior run as a baseline.
